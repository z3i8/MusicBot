  ...
  async handleAutoplayGenre(interaction, client) {
    ...
    if (player.voiceChannel.id !== member.voice.channel.id) {
      return await interaction.reply({
        content: await LanguageManager.getTranslation(
          guild?.id,
          "modalhandler.different_voice_channel"
        ),
        flags: [1 << 6],
      });
    }
    ...
    const genreName = await LanguageManager.getTranslation(
      guild?.id,
      ...
      .setTitle(
        "🎲 " +
          (await LanguageManager.getTranslation(
            guild?.id,
            "buttonhandler.autoplay_enabled"
          ))
      )
      .setDescription(
        (
          await LanguageManager.getTranslation(
            guild?.id,
            "buttonhandler.autoplay_enabled_desc"
          )
    ...
    if (success) {
      const embed = new EmbedBuilder()
        .setTitle(
          await LanguageManager.getTranslation(
            guild?.id,
            "modalhandler.volume_changed_title"
          )
        )
        .setDescription(
          await LanguageManager.getTranslation(
            guild?.id,
            "modalhandler.volume_changed_desc",
            { volume }
          )
        )
        .setColor(config.bot.embedColor)
